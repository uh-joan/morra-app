...
## The fits (`calibration/fit.ts`, pure, tested)

- `HIGH_V = clamp(max(2·jitterP95, 0.45·median(throwPeaks)), 0.3..1.5)`
...
- `vadMult = clamp(sqrt(median(shoutPeaks) / ambientFloor), 2.5..12)` — a
  36× shout over the floor lands on the spike's 6.
